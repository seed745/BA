...
for x_idx in x_grid[:-1]: 
    for i in range(20):
        for j in range(20):
            ...
            if x_idx >= x_start:  
                if x_idx < x_start + pixelsize_x:
                    bin = f"{int(np.round(x_start,0))}&{int(np.round(x_start+width,0))}"
                    if bin not in phi_min.keys():
                        phi_min[bin] = [[i,j]]

                    else:
                        phi_min[bin].append([i,j])  
"""             elif x_start >= 0 and x_idx >= x_start:
                if x_idx < x_start + pixelsize_x:
                    bin = f"{int(np.round(x_start,0))}-{int(np.round(x_start+width,0))}"
                    if bin not in phi_plus.keys():
                        phi_plus[bin] = [[i,j]]

                    else:
                        phi_plus[bin].append([i,j])  """
...
print(find_bigger_bins(phi_min))
print()
#find_bigger_bins(phi_plus)
